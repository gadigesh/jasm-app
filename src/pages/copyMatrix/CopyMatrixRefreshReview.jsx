...
import { useApplyCopyMatrixRefreshMutation } from "../../store/services/copyMatrix";
import {
	clearRefreshReview,
	readRefreshReview,
} from "../../utils/copyMatrixRefresh";
...
const CopyMatrixRefreshReview = () => {
	const { id } = useParams();
	const location = useLocation();
	const navigate = useNavigate();
	const [isApplying, setIsApplying] = useState(false);
	const [applyRefresh] = useApplyCopyMatrixRefreshMutation();

	const review = useMemo(
		() => location.state?.review || readRefreshReview(id) || {},
	...
	const goToEdit = (highlights = null) => {
		clearRefreshReview(id);
		navigate(`/copy-matrix/${id}/preview`, {
			replace: true,
			state: highlights
		...
		<div className="min-h-full bg-white px-4 py-4 sm:px-8">
			<div className="mx-auto max-w-[1180px]">
				<div className="mb-2 text-[10px] text-gray-400">
					Dashboard <span className="px-1">›</span> Copy Matrix{" "}
				...
				<div className="flex flex-wrap items-start justify-between gap-3 border-b border-gray-200 pb-4">
					<div>
						...
						<p className="mt-1 text-xs text-gray-500">
							Approve to apply source changes, or reject to keep
							the current copy matrix. Both open the edit page.
						</p>
					</div>
				...
				<section className="mb-5">
					<div className="mb-2">
						...
						<p className="text-[10px] text-gray-400">
							These values changed in the source file or Google Sheet.
						</p>
					</div>
					{changes.length > 0 ? (
...
export default CopyMatrixRefreshReview;
